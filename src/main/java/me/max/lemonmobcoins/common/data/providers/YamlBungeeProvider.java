...
public class YamlBungeeProvider implements DataProvider {
    ...
    public YamlBungeeProvider(File dataFolder) {
    ...
    public Map<UUID, Double> loadData() throws IOException {
        ...
        Configuration coinsData = ConfigurationProvider.getProvider(YamlConfiguration.class).load(file);
        coinsData.getKeys().forEach(key -> coins.put(UUID.fromString(key), coinsData.getDouble(key)));
    ...
    public void saveData(Map<UUID, Double> coins) throws IOException {
        ...
        Configuration coinsData = ConfigurationProvider.getProvider(YamlConfiguration.class).load(file);
        for (Map.Entry<UUID, Double> entry : coins.entrySet())
            coinsData.set(entry.getKey().toString(), entry.getValue());
        ConfigurationProvider.getProvider(YamlConfiguration.class).save(coinsData, file);
    }
}
